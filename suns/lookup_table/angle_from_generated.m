function [] = angle_from_generated(meas_file)
%PROCESS_SUNS_FILE Summary of this function goes here
%   Detailed explanation goes here

load('..\generate_suns_characteristics\generated_hires.mat');
...
for als=[1, 2, 3]
    als
    calibration_x = generated.valueVisNormalized(:, :, 1);
    calibration_y = generated.valueVisNormalized(:, :, 2);
...
mkdir(strcat('outputs_generated\', meas_file));
...
title(strcat(meas_file, ' SunS Exp - Theta, Cal:'));
ylabel('Theta Angle (\circ)');
xlabel('Time (min)');
legend(legend_items, 'Location', 'Best');
grid on;
saveas(f, strcat('outputs_generated\', meas_file, '\', meas_file, '_suns_exp_theta.png'));
...
title(strcat(meas_file, ' SunS Exp - Fi, Cal:'));
ylabel('Fi Angle (\circ)');
xlabel('Time (min)');
grid on;
saveas(f, strcat('outputs_generated\', meas_file, '\', meas_file, '_suns_exp_fi.png'));
close all;
...
suns_exp.calibration_file = "generated";


save(strcat('outputs_generated\', meas_file, '\', meas_file, '_suns_exp.mat'), 'suns_exp');
end
